...
- [x] Fixed VPN/Tailscale bypassing Traefik on Athena-hosted services — pointed gitea/semaphore.hughboi.cc's A records at dock-prod (10.10.10.10) instead of Athena directly, so every client (LAN/VPN/public/Athena-itself) routes through Traefik uniformly. IP:port (10.10.10.8:3000/:3001) still works as a fallback if Traefik is ever down. Hit and fixed two real bugs along the way: (1) hand-edited zone files with `update-policy` set are "dynamic" and silently ignore plain `rndc reload` — needs freeze/reload/thaw, see Terraform Bind9.md; (2) Athena's DNS resolver preference (bind9 first) was set via netplan, which cloud-init wipes on every reboot — moved to a systemd-resolved drop-in instead
	- [x] Add the systemd-resolved DNS drop-in to the `setup-athena` Ansible playbook so a rebuilt Athena gets it automatically instead of needing this manual step again
...
- [x] Actually configure Semaphore: SSH key, point at Gitea repo, inventory, task templates — fully tested end-to-end with a real playbook run (audit-users) against pve-srv-1, dock-prod, tailscale-gateway. Found + fixed 4 more real bugs along the way: Semaphore's UID/GID needed to be 1001:0 not 1000:1000 (image-specific, renamed to COMPOSE_UID/COMPOSE_GID to avoid bash's readonly $UID); ansible_become:false was missing across 16 more Proxmox-targeting inventories (no sudo binary on Proxmox); a shell exit-code bug in audit-users itself (last-command-in-a-loop pattern failed silently when a group had zero members); SSH-key paste corruption via terminal copy/paste (fixed via pbcopy piping instead)
