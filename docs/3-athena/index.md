...
> [!WARNING]
> **Pin Athena's own DNS resolver to bind9 explicitly — DHCP's server order isn't reliable, even
> when DHCP itself is configured correctly.** UniFi's DHCP DNS Server list for this VLAN is
> correctly ordered (`10.10.10.8, 10.10.10.10, 9.9.9.9` — bind9 first) — the bug is entirely on
> Athena's side. systemd-resolved doesn't treat DHCP's order as a strict preference; it picked
> `9.9.9.9` (Quad9) as `Current DNS Server` in practice, which obviously can't resolve
> internal-only hostnames like `gitea.hughboi.cc`. This breaks anything on Athena doing a plain
> hostname lookup (git, curl, etc.) with a confusing "does not have any RR of the requested type"
> error, even though `dig @10.10.10.8 <host>` (bypassing the system resolver) works fine the whole
> time.
>
> **Don't fix this via netplan's `nameservers:` block** — it doesn't survive a reboot. Athena's
> `/etc/netplan/50-cloud-init.yaml` is regenerated by cloud-init on every boot, silently wiping out
> any manual edit (confirmed: the fix vanished after a routine reboot for an unrelated CPU-type
> change). Use a **systemd-resolved drop-in** instead — cloud-init doesn't touch this path, so it
> survives reboots:
>
> ```sh
